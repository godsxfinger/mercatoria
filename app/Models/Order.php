...
class Order extends Model
{
    ...
    /**
     * Generate a crypto payment address for the order payment.
     * 
    ...
    public function markAsCompleted()
    {
        ...
        // Ensure stock is sufficient before processing vendor payout.
        foreach ($this->items as $item) {
            ...
            if ($product->stock_amount < $actualQuantity) {
                Log::warning("Order {$this->id} cannot complete due to insufficient stock for product {$product->id}");
                return false;
            }
        }

        // Process automatic payment to vendor first; do not complete order if payout fails.
        $paymentDetails = $this->processVendorPayment();
        ...
        try {
            DB::transaction(function () use ($paymentDetails) {
                ...
                foreach ($this->items as $item) {
                    ...
                    if ($product->stock_amount < $actualQuantity) {
                        throw new \RuntimeException("Stock changed during completion for order {$this->id}");
                    }
                ...
                $this->vendor_payment_amount = $paymentDetails['amount'];
                $this->vendor_payment_address = $paymentDetails['address'];
            ...
            Log::error("Failed to finalize completion state for order {$this->id} after payout: " . $e->getMessage());
            return false;
        }
    }
     ...
     * 
     * @return array{amount: float, address: string}|false
     */
    public function processVendorPayment()
    {
        // Only process payment if we have received crypto funds for this order
        ...
        if ($this->subtotal <= 0) {
            Log::error("Order {$this->id} has invalid subtotal for payout calculation");
            return false;
        }

        if ($this->commission < 0 || $this->commission > $this->subtotal) {
            Log::error("Order {$this->id} has invalid commission for payout calculation");
            return false;
        }
    ...
    {
        // Only process refund if we have received crypto funds for this order
        if (!$this->total_received_xmr || $this->total_received_xmr <= 0) {
    ...
    public function markAsCancelled()
    {
        ...
        $refundDetails = null;
        if ($currentStatus === self::STATUS_PAYMENT_RECEIVED ||
            $currentStatus === self::STATUS_PRODUCT_SENT ||
            $currentStatus === self::STATUS_DISPUTED) {
            $refundDetails = $this->processBuyerRefund();
            if ($refundDetails === false) {
                return false;
            }
        }

        try {
            DB::transaction(function () use ($currentStatus, $refundDetails) {
                ...
                if (is_array($refundDetails)) {
                    $this->buyer_refund_amount = $refundDetails['amount'];
                    $this->buyer_refund_address = $refundDetails['address'];
            ...
            Log::error("Failed to finalize cancellation state for order {$this->id}: " . $e->getMessage());
            return false;
        }
    }
    ...
    public function getFormattedStatus()
    {
        return match($this->status) {
            ...
            self::STATUS_REFUNDED => 'Order Refunded',
